"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateTransactionController = void 0;
const UpdateTransactionService_1 = require("../service/UpdateTransactionService");
class UpdateTransactionController {
    constructor() {
        this.updateTransactionService = new UpdateTransactionService_1.UpdateTransactionService();
    }
    handle(req, reply) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id, name, value } = req.query;
            try {
                // Assuming your service method returns the updated transaction
                const updatedTransaction = yield this.updateTransactionService.execute({ id, name, value });
                reply.send(updatedTransaction);
            }
            catch (error) {
                console.error("Error updating transaction:", error);
                reply.status(500).send({ error: "Internal Server Error" });
            }
        });
    }
}
exports.UpdateTransactionController = UpdateTransactionController;
